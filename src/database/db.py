...
# User operations
async def get_user(user_id: int, user_data: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
...
def format_user_display(user_data: Dict[str, Any]) -> str:
    """Format user data for display in admin panels and profiles"""
    try:
        ...
        db_logger.error(f"Error formatting user display: {e}")
        return f"👤 User {user_data.get('user_id', 'Unknown')}"

def extract_user_data_from_update(update) -> Dict[str, Any]:
    """Extract user data from Telegram update object"""
    try:
        user = None
        if update.message:
        ...
        db_logger.error(f"Error extracting user data from update: {e}")
        return {}